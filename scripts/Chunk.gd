...
func _greedy_mesh_axis(surface_tool: SurfaceTool, direction: Vector3i, face_type: String, neighbor_chunks: Dictionary):
	...
	for d in range(CHUNK_SIZE):
		...
		_greedy_mesh_slice(surface_tool, mask, d, direction, axis_d, axis_u, axis_v, face_type)

func _greedy_mesh_slice(surface_tool: SurfaceTool, mask: Array, depth: int, direction: Vector3i, axis_d: int, axis_u: int, axis_v: int, face_type: String):
	for v in range(CHUNK_SIZE):
		for u in range(CHUNK_SIZE):
			...
			_add_greedy_quad(surface_tool, pos_3d, width, height, direction, axis_u, axis_v, block_type, face_type)

			for dv in range(height):
				for du in range(width):
					mask[u + du + (v + dv) * CHUNK_SIZE] = null

func _add_greedy_quad(surface_tool: SurfaceTool, pos: Vector3i, width: int, height: int, normal: Vector3i, axis_u: int, axis_v: int, block_type: int, face_type: String):
